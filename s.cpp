...
int help(char * app)  
{  
    printf("怎么使用: %s TCP/SYN 开始的IP地址 [结束的IP地址] 要扫描的端口数 [线程数] [/T(N)] [/(H)Banner] [/Save]\n", app);  
    printf("示例: %s TCP 12.12.12.12 12.12.12.254 80 512\n", app);  
    printf("示例: %s TCP 12.12.12.12/24 80 512\n", app);  
    printf("示例: %s TCP 12.12.12.12/24 80 512 /T8 /Save\n", app);  
    printf("示例: %s TCP 12.12.12.12 12.12.12.254 80 512 /HBanner\n", app);  
    printf("示例: %s TCP 12.12.12.12 12.12.12.254 21 512 /Banner\n", app);  
  
    printf("示例: %s TCP 12.12.12.12 1-65535 512\n", app);  
    printf("示例: %s TCP 12.12.12.12 12.12.12.254 21,3389,5631 512\n", app);  
    printf("示例: %s TCP 12.12.12.12 21,3389,5631 512\n", app);  
    printf("示例: %s SYN 12.12.12.12 12.12.12.254 80\n", app);  
    printf("示例: %s SYN 12.12.12.12 1-65535\n", app);  
    printf("示例: %s SYN 12.12.12.12 12.12.12.254 21,80,3389\n", app);  
    return printf("示例: %s SYN 12.12.12.12 21,80,3389\n", app);  
}  
...
{  
  ...
  char  realEndIpAddr[200];  
    ...
    {  
        ...
        if ( !isWin2K() )  
        {  
            printf("SYN 扫描只能在Windows 2000以上的系统运行\n");  
            return 0;  
        }  
        _maxThreads = 1;  
    }  
  ...
  {  
    ...
    {  
      ...
      {  
        if ( isSynScan )  
        {  
          printf("SYN 扫描: 正在扫描 %u IP地址 ， 使用了 %d 线程\n", _endIp - _startIp + 1, _maxThreads);  
          if ( _isLog )  
          {  
            wsprintf(  
              buf,  
              "SYN 扫描: 正在扫描 %u IP地址 ， 使用了 %d 线程\r\n",  
              _endIp - _startIp + 1,  
              _maxThreads);  
        ...
        {  
          printf("正常扫描: 正在扫描 %u IP地址 ， 使用了 %d 线程\n", _endIp - _startIp + 1, _maxThreads);  
          if ( _isLog )  
          {  
            wsprintf(  
              buf,  
              "正常扫描: 正在扫描 %u IP地址 ， 使用了 %d 线程\r\n",  
              _endIp - _startIp + 1,  
              _maxThreads);  
            logWriteBuffer(_logFile, buf);  
          }  
      ...
      {  
          ...
          {  
            ...
            {  
              ...
              {  
                if ( _isMultiplePort )  
                {  
                  printf("SYN 扫描: 将使用%d线程 来扫描 %s:%d\n", startIpAddr, *(DWORD *)_portsArray, _maxThreads);  
                  if ( _isLog )  
                  {  
                    wsprintf(  
                      buf,  
                      "SYN 扫描: 将使用%d线程 来扫描 %s:%d\r\n",  
                      startIpAddr,  
                      *(DWORD *)_portsArray,  
                      _maxThreads);  
                    logWriteBuffer(_logFile, buf);  
              ...
              {  
                if ( _isMultiplePort )  
                {  
                  printf("Normal Scan: 将使用%d线程 来扫描 %s:%d\n", startIpAddr, *(DWORD *)_portsArray, _maxThreads);  
                  if ( _isLog )  
                  {  
                    wsprintf(  
                      buf,  
                      "Normal Scan: 将使用%d线程 来扫描 %s:%d\r\n",  
                      startIpAddr,  
                      *(DWORD *)_portsArray,  
                      _maxThreads);  
                    logWriteBuffer(_logFile, buf);  
        ...
        {  
          ...
          {  
            if ( _isMultiplePort )  
            {  
              printf("SYN Scan: 正在扫描 %u 端口 使用了 %d 线程\n", _portsTotal, _maxThreads);  
              if ( _isLog )  
              {  
                wsprintf(buf, "SYN Scan: 正在扫描 %u 端口 使用了 %d 线程\r\n", _portsTotal, _maxThreads);  
                logWriteBuffer(_logFile, buf);  
              }  
            }  
            else  
          ...
          {  
            if ( _isMultiplePort )  
            {  
              printf("Normal Scan: 正在扫描 %u 端口 使用了 %d 线程\n", _portsTotal, _maxThreads);  
              if ( _isLog )  
              {  
                wsprintf(buf, "正在扫描 %u 端口 使用了 %d 线程", _portsTotal, _maxThreads);  
                logWriteBuffer(_logFile, buf);  
              }  
            }  
            else  
            {  
              printf(  
                "Normal Scan: 正在扫描 %u 端口 使用了 %d 线程\n",  
                dword_407090 - _portScanSingle + 1,  
                _maxThreads);  
              if ( _isLog )  
              {  
    ...
    {  
      ...
      {  
        ...
        {  
          printf(  
            "SYN Scan: 正在扫描 %u IP地址 开放的 %u 端口 使用 %d 线程\n",  
            _endIp - _startIp + 1,  
            _portsTotal,  
            _maxThreads);  
          if ( _isLog )  
      ...
      {  
        ...
        {  
          printf(  
            "Normal Scan: 正在扫描 %u IP地址 开放的 %u 端口 使用 %d 线程\n",  
            _endIp - _startIp + 1,  
            _portsTotal,  
            _maxThreads);  
          if ( _isLog )  
        ...
        {  
          ...
          {  
            ...
            *((DWORD *)lpParameter + 1) = *((DWORD *)_portsArray + portIndex);  
            ...
            {  
              EnterCriticalSection(&_cs);  
              printf("%u IP地址 扫描完成.使用了 %d 线程 \r", _ipScanned / (unsigned int)_portsTotal, _threadsUsed++);  
              ++count;  
              LeaveCriticalSection(&_cs);  
              CloseHandle(hObject);  
            }  
          ...
          {  
            ...
            *((DWORD *)param + 1) = portIndex;  
            ...
            {  
              ...
              {  
                if ( !_isSinglePort )  
                  printf("%u 端口 扫描完成.使用了 %d 端口 \r", _ipScanned, _threadsUsed);  
              }  
              else  
              {  
                v23 = _ipScanned / (unsigned int)lpParameter;  
                printf("%u IP地址 扫描完成.使用了 %d 线程 \r", _ipScanned / (unsigned int)lpParameter, _threadsUsed);  
              }  
              ++_threadsUsed;  
              ++count;  
              LeaveCriticalSection(&_cs);  
...
LABEL_147:  
    ...
    {  
      WaitForSingleObject(_semaphore, INFINITE);  
      EnterCriticalSection(&_cs);  
      printf("%d 线程 在扫描中......              \r", _threadsUsed);  
    ...
    {  
      ...
      {  
        printf(  
          "扫描 %s 已经完成 使用了 %d 个小时 %d 分钟 %d 秒. 找到了 %u 个开放的端口\n",  
          startIpAddr,  
          hoursElapsed,  
          minutesElapsed,  
          secondsElapsed,  
      ...
      {  
        ...
        printf(  
          "扫描 %s 已经完成 使用了 %d 个小时 %d 分钟 %d 秒. 找到了 %u 个开放的主机\n",  
          lpParameter,  
          hoursElapsed,  
          minutesElapsed,  
          secondsElapsed,  
...
{  
    int ret;  
    printf("TCP Port Scanner By WinEggDrop 编译 Jian 2018/6/27 ＊﹏塟じò ぴé家族冷少〆… 专版\n");  
